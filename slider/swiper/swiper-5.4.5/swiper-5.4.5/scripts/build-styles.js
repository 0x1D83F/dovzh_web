...
async function build(cb) {
  ...
  let lessContent = fs.readFileSync(path.resolve(__dirname, '../src/swiper_main.less'), 'utf8');
  ...
  // Copy less & scss
  const iconsFontBase64 = base64Encode('./src/icons/font/swiper_main-icons.woff');
  ...
  glob('**/*.*', { cwd: path.resolve(__dirname, '../src') }, (err, files) => {
    files.forEach((file, index) => {
      ...
      if (file.indexOf('swiper_main.less') >= 0) {
  ...
  // Write file
  fse.writeFileSync('./package/css/swiper_main.min.scss', `${banner}\n${minifiedContent}`);
